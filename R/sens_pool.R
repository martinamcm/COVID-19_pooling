###############################################################
# Script to generate plots for pooled sensitivity estimates
#
###############################################################

# 1. Load data ---------------------------------------------------------------

# Load original sensitivity data from Rivett L, Sridhar S, Sparkes D, et al. 
...
## Figure: Plot pooled sens vs prevalence by pool size

datapoolsubn <- datapool[datapool$npool<=5,]

ggplot(
  datapoolsubn,
  aes(x = Prevalence,y = Sensitivity, group = as.factor(npool), color = as.factor(npool))
  ) +
  geom_line() + 
  geom_ribbon(
    aes(ymin = SensCIlow, ymax = SensCIupp, color = as.factor(npool)),color=NA,alpha=0.1
    ) +
  ylab("Pooled sensitivity") + 
  xlab("Prevalence") + 
  ylim(c(0.7,1)) +
  theme_bw() + 
  ...
  labs(color=expression(n[pool]))


## Figure: Plot pooled sens vs pool size by prevalence
...
ggsens <- 
  ggplot(
    datapoolsubp,
    aes(x = npool, y = Sensitivity, group = as.factor(Prevalence), color = as.factor(Prevalence))
    ) +
  geom_line() + 
  geom_ribbon(
    aes(ymin = SensCIlow, ymax = SensCIupp, color = as.factor(Prevalence)), color = NA, alpha = 0.1
    ) +
  ylab("Pooled sensitivity") +
  ...
  scale_color_viridis(discrete=TRUE,labels=label.sens)



## Figure: Plot pooled sensitivity for 70%,80%,90%
...
ggpool <- 
  ggplot(
    data_sens,
    aes(x=npool,y=poolsens,group=as.factor(sensnopool),color=as.factor(sensnopool))
    ) +
  geom_line() + 
  geom_ribbon(
    aes(ymin=poolsensL, ymax=poolsensH,color=as.factor(sensnopool)),color=NA,alpha=0.1
    ) +
  ylab("Pooled sensitivity") +
